...
let processLogs = []; // Make it local to module, not global
const MAX_LOGS = 500;
...
const stopProcess = () => {
	if (activeProcess) {
		...
		// Clear timeout if process exits before 5 seconds - FIX
		const clearTimeoutOnExit = () => {
			clearTimeout(killTimeout);
		};
		
		if (activeProcess.once) {
			activeProcess.once('exit', clearTimeoutOnExit);
		}
	}
};
