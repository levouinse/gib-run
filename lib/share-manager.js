...
const { createCleanupTimer } = require('./utils');

const shareLinks = new Map();
const MAX_SHARE_LINKS = 100;

// Periodic cleanup to prevent memory leak
const cleanupTimer = createCleanupTimer(() => {
	cleanupExpiredLinks();
}, 60000); // 1 minute

// Unref timer to allow process to exit cleanly
...
const createShareLink = (options = {}) => {
	...
	const plainPassword = options.password || (options.secure ? generatePassword() : null);
	const expiresAt = options.expiresIn ? Date.now() + options.expiresIn : null;
	
	const shareData = {
		token,
		password: plainPassword, // Store plain for display
		passwordHash: plainPassword ? hashPassword(plainPassword) : null, // Store hash for validation
		createdAt: Date.now(),
		expiresAt,
		accessCount: 0,
...
const validateShareLink = (token, password) => {
	// FIX: Input validation to prevent attacks
	if (!token || typeof token !== 'string' || token.length !== 32 || !/^[a-f0-9]{32}$/.test(token)) {
	...
	if (shareData.passwordHash) {
		...
		const storedHash = shareData.passwordHash;
...
const cleanupExpiredLinks = () => {
	...
	const entries = Array.from(shareLinks.entries());
